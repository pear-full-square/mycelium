...
export function createCrud (gitLayer) {
  ...
  return {
    ...
    // List entries at path. Returns array of names. Throws if not found or not a directory.
    async list (path, ref) {
      const rootTree = await getHeadTree(ref)
      if (path === '/' || path === '') {
        return (await gitLayer.readTree(rootTree)).map(e => e.path)
      }
      const resolved = await gitLayer.resolvePath(rootTree, path)
      if (!resolved) throw new Error(`not found: ${path}`)
      if (resolved.type !== 'tree' && resolved.mode !== '040000') {
      ...
      return (await gitLayer.readTree(resolved.oid)).map(e => e.path)
    },
